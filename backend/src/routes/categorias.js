...
router.post('/', async (req, res) => {
  try {
    ...
    const existente = await query('SELECT id FROM categorias WHERE nombre = $1', [nombre])
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const result = await query('UPDATE categorias SET nombre = $1 WHERE id = $2 RETURNING *', [nombre, req.params.id])
